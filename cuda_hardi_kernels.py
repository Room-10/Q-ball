...
def prepare_const_gpudata(b_sph, f, Y, M, constraint_u, uconstrloc):
...
def prepare_kernels(u1, u2, v, w, b_sph, avgskips):
    ...
    kernels_code += open('cuda_hardi_kernels_primal.cu', 'r').read()
    kernels_code += open('cuda_hardi_kernels_dual.cu', 'r').read()
    ...
    [k['func'].prepare("P"*25 + "d"*5 + "P"*2) for k in result]
