...
class MyPDHGModel(PDHGModelHARDI):
    ...
    def prepare_gpu(self):
        ...
        self.cuda_files = [
            resource_stream('qball.solvers.sh_bndl2_tvw', 'cuda_primal.cu'),
            resource_stream('qball.solvers.sh_bndl2_tvw', 'cuda_dual.cu'),
        ]
    ...
    def obj_primal(self, x, ygrad):
        ...
        obj_p = 0.5*np.einsum('k,ki->', c['b'], np.fmax(0.0, flmu)**2) \
              + 0.5*np.einsum('k,ki->', c['b'], np.fmax(0.0, umfu)**2) \
              + c['lbd']*e['g_norms'].sum()
    ...
    def obj_dual(self, xgrad, y):
        ...
        obj_d = -np.sum((
              ...
              )[:,c['inpaint_nloc']]) - np.sum(q0)*c['b_precond']
        ...
        u2[:,c['inpaint_nloc']] = (1.0/(1.0 + u2tau*c['b'][:,None]) * \
            np.fmax(u2 + u2tau*c['b'][:,None]*c['fl'],
                np.fmin(u2 + u2tau*c['b'][:,None]*c['fu'],
                    (1.0 + u2tau*c['b'][:,None])*u2)))[:,c['inpaint_nloc']]
